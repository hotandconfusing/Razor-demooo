...
namespace VladislavTsurikov.MegaWorld.Editor.Common.Settings.FilterSettings.MaskFilterSystem
{
    ...
    public class AspectFilterEditor : ReorderableListComponentEditor
    {
        ...
        public override void OnGUI(Rect rect, int index)
        {
            if (index != 0)
            {
                _aspectFilter.BlendMode = (BlendMode)CustomEditorGUI.EnumPopup(
            ...
            _aspectFilter.RemapCurve =
                CustomEditorGUI.CurveField(new Rect(rect.x, rect.y, rect.width, EditorGUIUtility.singleLineHeight),
