...
{
    public static class SpawnFitness
    {
        ...
        public static float Get(Group group, RayHit rayHit)
        {
            ...
            if (filterSettings.FilterType == FilterType.MaskFilter)
            {
                return MaskFilterTextureCache.GetFitness(filterSettings.MaskFilterComponentSettings, rayHit.Point);
            }

            return GetFromSimpleFilter(filterSettings.SimpleFilter, rayHit);
        }

        public static float Get(MaskFilterComponentSettings maskFilterComponentSettings, Terrain terrain,
            Vector3 point)
        {
            return MaskFilterTextureCache.GetFitness(maskFilterComponentSettings, terrain, point);
        }
