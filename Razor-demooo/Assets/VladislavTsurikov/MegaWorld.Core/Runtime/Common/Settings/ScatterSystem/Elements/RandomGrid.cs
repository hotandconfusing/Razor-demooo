...
namespace VladislavTsurikov.MegaWorld.Runtime.Common.Settings.ScatterSystem
{
    ...
    [Name("Random Grid")]
    [MegaWorldDocUrl("scatter-system/random-grid")]
    public class RandomGrid : Scatter
    {
            ...
            Action<Vector3> onSpawn = null)
        {
            ...
            for (position.z = gridOrigin.z; position.z < maxPosition.y; position.z += GridStep.y)
            {
                ...
                Vector2 offsetLocalPosition2D = new(_visualOrigin.x + newLocalPosition.x,
                ...
                {
                    Vector3 worldPosition = new(offsetLocalPosition2D.x, boxArea.Center.y, offsetLocalPosition2D.y);
        ...
        {
            Vector3 gridOrigin = Vector3.zero;
            Vector3 localGridStep = new(GridStep.x, GridStep.y, 1);
            Vector3 gridNormal = Vector3.up;

            float halfSpawnRange = size / 2;

            Vector3 point = new(dragPoint.x - halfSpawnRange, 0, dragPoint.z - halfSpawnRange);
        ...
        private Vector2 GetRandomSquarePoint(Vector2 sample)
        {
            ...
            Vector2 distanceOffset = new(Random.Range(-halfDistanceX, halfDistanceX),
        ...
        private Vector2 GetRandomSpherePoint(Vector2 sample)
        {
            ...
            Vector2 distanceOffset = new(Random.Range(-halfDistance, halfDistance),
