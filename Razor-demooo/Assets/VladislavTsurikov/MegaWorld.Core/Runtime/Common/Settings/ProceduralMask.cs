...
namespace VladislavTsurikov.MegaWorld.Runtime.Common.Settings
{
    ...
    public class ProceduralMask
    {
        ...
        [NonSerialized]
        private Shape _cachedShape = Shape.Circle;

        private Texture2D _mask;

        public Texture2D GetMask(Shape shape)
        {
            if (_mask == null || _cachedShape != shape)
            {
                CreateProceduralTexture(shape);
            }

            return _mask;
        }

        public void CreateProceduralTexture(Shape shape)
        {
            _cachedShape = shape;
                ...
                for (int x = 0; x < width; x++)
                {
                    switch (shape)
                    {
                        ...
                        case Shape.Square:
                        {
                            ...
                            Bounds originBoundsSize = new(center, new Vector3(newMinSize, newMinSize, newMinSize));
                            Bounds offsetBoundsSize = new(center, new Vector3(size, size, size));

                            float maxDifference = offsetBoundsSize.extents.x - originBoundsSize.extents.x;
                            float minDifference =
                                Vector3.Distance(checkPoint, originBoundsSize.ClosestPoint(checkPoint));
            ...
            for (int y = 0; y < height; y++)
            {
                for (int x = 0; x < width; x++)
                {
                    ...
                    _mask.SetPixel(x, y, new Color(fractalValue, fractalValue, fractalValue, 1));
                }
            }
        ...
        {
            FractalNoiseCPU fractal = new(GetNoiseForProceduralBrush(), Octaves, Frequency / 7, Lacunarity,
                Persistence);
