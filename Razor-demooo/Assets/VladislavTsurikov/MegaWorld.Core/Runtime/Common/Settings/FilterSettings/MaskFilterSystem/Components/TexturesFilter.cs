...
namespace VladislavTsurikov.MegaWorld.Runtime.Common.Settings.FilterSettings.MaskFilterSystem
{
    [Serializable]
    [Name("Textures")]
    [MegaWorldDocUrl("mask-filters/textures-filter")]
    public class TexturesFilter : MaskFilter
    {
        ...
        public override void Eval(MaskFilterContext maskFilterContext, int index)
        {
            ...
            List<TerrainTexture> addTexturesToRenderTextureList = new();

            if (!IsSyncTerrain(Terrain.activeTerrain))
            {
            ...
            RenderTexture output = RenderTexture.GetTemporary(maskFilterContext.SourceRenderTexture.width,
            ...
            {
                RenderTexture localSourceRender = RenderTexture.GetTemporary(
                    maskFilterContext.SourceRenderTexture.width,
            ...
            RenderTexture sourceRender = RenderTexture.GetTemporary(maskFilterContext.SourceRenderTexture.width,
                maskFilterContext.SourceRenderTexture.height, 1, RenderTextureFormat.ARGB32);
        ...
        public void UpdateCheckTerrainTextures(Terrain activeTerrain)
        {
            ...
            for (idx = 0; idx < terrainLayers.Length; idx++)
            {
                ...
                TerrainTexture checkTerrainTextures = new()
