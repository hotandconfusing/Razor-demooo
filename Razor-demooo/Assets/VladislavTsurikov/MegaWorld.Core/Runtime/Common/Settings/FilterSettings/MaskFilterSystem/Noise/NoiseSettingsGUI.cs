...
namespace VladislavTsurikov.MegaWorld.Runtime.Common.Settings.FilterSettings.MaskFilterSystem.Noise
{
    public class NoiseSettingsGUI
    {
        ...
        private void HandlePreviewTextureInput(Rect previewRect)
        {
            ...
            Vector2 previewDims = new(previewRect.width, previewRect.height);
            Vector2 abs = new(Mathf.Abs(s.x), Mathf.Abs(s.z));
            ...
            else if (draggingPreview)
            {
                ...
                Vector2 sign = new(-Mathf.Sign(s.x), Mathf.Sign(s.z));
                Vector2 delta = Event.current.delta / previewDims * abs * sign;
                Vector3 d3 = new(delta.x, 0, delta.y);
        ...
        public void DrawPreviewTexture(ref Rect rect, float minSize, bool showExportButton = true)
        {
            ...
            if (MNoiseSettings.ShowNoisePreviewTexture)
            {
                ...
                Rect totalRect = new(currentRect.x, currentRect.y, currentRect.width, size);
                ...
                {
                    Rect infoIconRect = new(totalRect.x + padding, totalRect.y + padding, iconWidth, iconWidth);
                ...
                Rect previewRect = new(totalRect.x + totalRect.width / 2 - minWidth / 2,
