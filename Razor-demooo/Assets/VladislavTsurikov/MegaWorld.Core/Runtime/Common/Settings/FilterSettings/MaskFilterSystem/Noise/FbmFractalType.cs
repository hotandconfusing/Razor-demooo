...
namespace VladislavTsurikov.MegaWorld.Runtime.Common.Settings.FilterSettings.MaskFilterSystem.Noise
{
    ...
    public class FbmFractalType : FractalType<FbmFractalType>
    {
        ...
        {
            return new FractalTypeDescriptor {
                Name = "Fbm",
                ...
                AdditionalIncludePaths = new List<string> { MaskFilterShadersPath.Path + "/NoiseLib/NoiseCommon.hlsl" }
            };
        }

        public override string GetDefaultSerializedString()
        {
            FbmFractalInput fbm = new();
        ...
        private void DomainWarpSettingsGUI(ref FbmFractalInput fbm)
        {
            ...
            {
                fbm.WarpIterations = CustomEditorGUILayout.Slider(Styles.WarpIterations, fbm.WarpIterations,
                    fbm.WarpIterationsMinMax.x, fbm.WarpIterationsMinMax.y);
                fbm.WarpStrength = CustomEditorGUILayout.Slider(Styles.WarpStrength, fbm.WarpStrength,
                    fbm.WarpStrengthMinMax.x, fbm.WarpStrengthMinMax.y);
                fbm.WarpOffsets = CustomEditorGUILayout.Vector4Field(Styles.WarpOffsets, fbm.WarpOffsets);
            }
        }
