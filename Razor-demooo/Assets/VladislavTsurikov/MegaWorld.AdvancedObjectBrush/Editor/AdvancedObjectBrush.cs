...
namespace VladislavTsurikov.MegaWorld.Editor.AdvancedBrushTool
{
    ...
    [AddToolComponents(new[] { typeof(AdvancedObjectBrushSettings), typeof(BrushSettings) })]
    ...
    [MegaWorldDocUrl("tools/object-brush")]
    public class AdvancedObjectBrush : ToolWindow
    {
        ...
        {
            _advancedObjectBrushSettings =
                (AdvancedObjectBrushSettings)ToolsComponentStack.GetElement(typeof(AdvancedObjectBrush),
                    typeof(AdvancedObjectBrushSettings));
            _brushSettings =
                (BrushSettings)ToolsComponentStack.GetElement(typeof(AdvancedObjectBrush), typeof(BrushSettings));
        ...
        private void OnMouseDrag(Vector3 dragPoint)
        {
            foreach (Group group in WindowData.Instance.SelectedData.SelectedGroupList)
            {
                ...
                if (rayHit != null)
                {
                    ...
                    if (area?.RayHit != null)
                    {
                        PaintGroup(group, area, _advancedObjectBrushSettings.EnableFailureRateOnMouseDrag);
                    }
                }
            }
        }
        ...
        private void OnRepaint()
        {
            ...
            AdvancedObjectBrushVisualisation.Draw(area, WindowData.Instance.SelectionData,
        ...
        {
            bool ShouldSkipSample() =>
                dragMouse && Random.Range(0f, 100f) < _advancedObjectBrushSettings.FailureRateOnMouseDrag;

            if (group.PrototypeType == typeof(PrototypeGameObject))
            {
                GameObjectSpawner.SpawnGroupAsync(CancellationToken.None, group, area, false,
                    ShouldSkipSample, true, true,
                    true).Forget();
            }
#if RENDERER_STACK
            else if (group.PrototypeType == typeof(PrototypeTerrainObject))
            {
                TerrainObjectSpawner.SpawnGroupAsync(CancellationToken.None, group, area, false,
                    ShouldSkipSample, true, true,
                    true).Forget();
            }
#endif
