...
namespace VladislavTsurikov.UnityUtility.Runtime
{
    public static class TextureUtility
    {
        public static float GetFromWorldPosition(Bounds bounds, Vector3 worldPosition, Texture2D texture)
        {
            ...
            float inverseY = Mathf.InverseLerp(bounds.center.z - bounds.extents.z, bounds.center.z + bounds.extents.z,
                worldPosition.z);
            float inverseX = Mathf.InverseLerp(bounds.center.x - bounds.extents.x, bounds.center.x + bounds.extents.x,
                worldPosition.x);
        ...
        public static float GetFromWorldPosition(Bounds bounds, Vector3 worldPosition, Texture texture)
        {
            return GetFromWorldPosition(bounds, worldPosition, texture as Texture2D);
        }
        ...
        public static float Get(Vector2 normal, Texture texture)
        {
            return Get(normal, texture as Texture2D);
        }
        ...
        public static Vector2 AnimationCurveToTexture(AnimationCurve curve, ref Texture2D tex)
        {
            ...
            Vector2 range = new(val, val);

            Color[] pixels = new Color[tex.width * tex.height];
            pixels[0].r = val;
            for (int i = 1; i < tex.width; i++)
            {
                float pct = i / (float)tex.width;
        ...
        {
            Texture2D tex = new(rTex.width, rTex.height, TextureFormat.ARGB32, false);
        ...
        public static float GetAlpha(Vector2 pos, Vector2 size, Texture mask)
        {
            return GetAlpha(pos, size, mask as Texture2D);
        }
