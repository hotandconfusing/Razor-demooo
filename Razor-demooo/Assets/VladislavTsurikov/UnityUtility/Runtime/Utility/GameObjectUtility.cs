...
namespace VladislavTsurikov.UnityUtility.Runtime
{
    public static class GameObjectUtility
    {
        ...
        public static GameObject FindParentGameObject(string gameObjectName, Scene scene)
        {
            ...
            if (container == null)
            {
                GameObject childObject = new(gameObjectName);
        ...
        {
            List<GameObject> roots = new(Mathf.Max(1, scene.rootCount));
            scene.GetRootGameObjects(roots);
            List<GameObject> sceneObjects = new(Mathf.Max(1, scene.rootCount * 5));
        ...
        {
            Bounds combinedbounds = new(go.transform.position, Vector3.zero);
