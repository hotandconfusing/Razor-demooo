...
namespace OdinSerializer
{
    ...
    public static class FormatterEmitter
    {
            ...
            if (!Formatters.TryGetInnerValue(policy, type, out result))
            {
                lock (LOCK)
                {
                    if (!Formatters.TryGetInnerValue(policy, type, out result))
                    {
                        ...
                        {
                            Debug.LogError("The following error occurred while emitting a formatter for the type " +
                                           type.Name);
        ...
        private static readonly DoubleLookupDictionary<ISerializationPolicy, Type, IFormatter> Formatters = new();

        private static AssemblyBuilder runtimeEmittedAssembly;
        private static ModuleBuilder runtimeEmittedModule;

        public delegate void ReadDataEntryMethodDelegate<T>(ref T value, string entryName, EntryType entryType,
            IDataReader reader);
        ...
        private static void EnsureRuntimeAssembly()
        {
            ...
            {
                AssemblyName assemblyName = new(RUNTIME_EMITTED_ASSEMBLY_NAME);
        ...
        /// <summary>
        ///     Emits a formatter for a given type into a given module builder, using a given serialization policy to determine
        ///     which members to serialize.
        /// </summary>
        ...
        public static Type EmitAOTFormatter(Type formattedType, ModuleBuilder moduleBuilder,
            ISerializationPolicy policy)
        {
            ...
            // Read
            {
                ...
                readBaseMethod.GetParameters()
                    .ForEach(n => readMethod.DefineParameter(n.Position, n.Attributes, n.Name));
                EmitReadMethodContents(readMethod.GetILGenerator(), formattedType, dictField, serializerFields,
                    memberNames, serializerReadMethods);
            }
            ...
            // Write
            {
                ...
                writeBaseMethod.GetParameters().ForEach(n =>
                    dynamicWriteMethod.DefineParameter(n.Position + 1, n.Attributes, n.Name));
                EmitWriteMethodContents(dynamicWriteMethod.GetILGenerator(), formattedType, serializerFields,
                    memberNames, serializerWriteMethods);
            }

            Type result = formatterType.CreateType();

            // Register the formatter on the assembly
            ((AssemblyBuilder)moduleBuilder.Assembly).SetCustomAttribute(new CustomAttributeBuilder(
                typeof(RegisterFormatterAttribute).GetConstructor(new[] { typeof(Type), typeof(int) }),
                new object[] { formatterType, -1 }));

            return result;
        }


        private static IFormatter CreateGenericFormatter(Type formattedType, ModuleBuilder moduleBuilder,
            ISerializationPolicy policy)
        {
            Dictionary<string, MemberInfo> serializableMembers =
                FormatterUtilities.GetSerializableMembersMap(formattedType, policy);
            ...
            if (serializableMembers.Count == 0)
            {
                return (IFormatter)Activator.CreateInstance(
                    typeof(EmptyTypeFormatter<>).MakeGenericType(formattedType));
            }
            ...
            // Read
            {
                ...
                DynamicMethod dynamicReadMethod = new("Dynamic_" + formattedType.GetCompilableNiceFullName(), null,
                    readDataEntryMethod.GetParameters().Select(n => n.ParameterType).ToArray(), true);
                readDataEntryMethod.GetParameters()
                    .ForEach(n => dynamicReadMethod.DefineParameter(n.Position, n.Attributes, n.Name));
                EmitReadMethodContents(dynamicReadMethod.GetILGenerator(), formattedType, dictField, serializerFields,
                    memberNames, serializerReadMethods);
            ...
            // Write
            {
                ...
                DynamicMethod dynamicWriteMethod = new("Dynamic_Write_" + formattedType.GetCompilableNiceFullName(),
                    null, writeDataEntriesMethod.GetParameters().Select(n => n.ParameterType).ToArray(), true);
                writeDataEntriesMethod.GetParameters().ForEach(n =>
                    dynamicWriteMethod.DefineParameter(n.Position + 1, n.Attributes, n.Name));
                EmitWriteMethodContents(dynamicWriteMethod.GetILGenerator(), formattedType, serializerFields,
                    memberNames, serializerWriteMethods);
            ...
            out Dictionary<MemberInfo, List<string>> memberNames)
        {
            TypeBuilder helperTypeBuilder =
                moduleBuilder.DefineType(helperTypeName,
                    TypeAttributes.Public | TypeAttributes.Sealed | TypeAttributes.Class);
            ...
            foreach (KeyValuePair<string, MemberInfo> entry in serializableMembers)
            {
                ...
                if (!memberNames.TryGetValue(entry.Value, out list))
                {
            ...
            dictField =
                helperTypeBuilder.DefineField("SwitchLookup", typeof(Dictionary<string, int>),
            ...
            foreach (Type t in neededSerializers)
            {
                ...
                serializerWriteMethods.Add(t,
                    serializerType.GetMethod("WriteValue", Flags.InstancePublicDeclaredOnly, null,
                        new[] { typeof(string), t, typeof(IDataWriter) }, null));
                serializerFields.Add(t,
                    helperTypeBuilder.DefineField(name, serializerType,
                        FieldAttributes.Public | FieldAttributes.Static | FieldAttributes.InitOnly));
            }
            ...
            {
                MethodInfo addMethod = typeof(Dictionary<string, int>).GetMethod("Add", Flags.InstancePublic);
                ConstructorInfo dictionaryConstructor = typeof(Dictionary<string, int>).GetConstructor(Type.EmptyTypes);
                MethodInfo serializerGetMethod =
                    typeof(Serializer).GetMethod("Get", Flags.StaticPublic, null, new[] { typeof(Type) }, null);
                MethodInfo typeOfMethod =
                    typeof(Type).GetMethod("GetTypeFromHandle", Flags.StaticPublic, null,
                ...
                foreach (KeyValuePair<MemberInfo, List<string>> entry in memberNames)
                {
                ...
                foreach (KeyValuePair<Type, FieldBuilder> entry in serializerFields)
                {
                    gen.Emit(OpCodes.Ldtoken, entry.Key); // Load type token
                    gen.Emit(OpCodes.Call,
                        typeOfMethod); // Call typeof method (this pushes a type value onto the stack)
            ...
            Dictionary<Type, MethodInfo> serializerReadMethods)
        {
            ...
            foreach (MemberInfo member in memberNames.Keys)
            {
                ...
                gen.Emit(OpCodes.Ldsfld,
                    serializerFields[memberType]); // Load serializer from serializer container type
                gen.Emit(OpCodes.Ldarg, (short)3); // Load reader argument
                gen.Emit(OpCodes.Callvirt,
                    serializerReadMethods[memberType]); // Call Serializer.ReadValue(IDataReader reader)
            ...
            foreach (MemberInfo member in memberNames.Keys)
            {
                ...
                else if (member is PropertyInfo)
                {
                    ...
                    else
                    {
                        ...
                        gen.Emit(OpCodes.Ldind_Ref); // Indirectly load value of reference
                        gen.Emit(OpCodes.Callvirt,
                            propInfo.DeAliasProperty().GetGetMethod(true)); // Call property getter
                    }
                }
                else
                {
                ...
                gen.Emit(OpCodes.Callvirt,
                    serializerWriteMethods[
                        memberType]); // Call Serializer.WriteValue(string name, T value, IDataWriter writer)
            }
