    ...
    public static class UnitySerializationUtility
    {
            ...
            bool serializeUnityFields = false, SerializationContext context = null)
        {
...

            {
                ...
                {
                    Object prefab = null;
                    SerializationData prefabData = default;
                            ...
                            lock (PrefabDeserializeUtility.DeserializePrefabs_LOCK)
                            {
                                    ...
                                    data.PrefabModificationsReferencedUnityObjects.Count > 0)
                                {
                                    ...
                                    foreach (Object reference in data.PrefabModificationsReferencedUnityObjects)
                                    {
                                        ...
                                        GameObject gameObject = (GameObject)(reference is GameObject
                                            ? reference
                ...
                else
                {
                    if (format == DataFormat.Nodes)
                    {
                        ...
                        if (context == null)
                        {
                            ...
                            using (SerializationNodeDataWriter writer = new(newContext))
                        ...
                        {
                            using (SerializationNodeDataWriter writer = new(context))
                            using (Cache<UnityReferenceResolver> resolver = Cache<UnityReferenceResolver>.Claim())
                            {
                                if (data.SerializationNodes != null)
                                {
        ...
        private static void SetUnityObjectModifications(Object unityObject, ref SerializationData data, Object prefab)
        {
            ...
            else
            {
                ...
                string referencedUnityObjectsPath = serializedDataPath +
                                                    SerializationData
                                                        .PrefabModificationsReferencedUnityObjectsFieldName +
                                                    ".Array.";
                string modificationsPath =
                    serializedDataPath + SerializationData.PrefabModificationsFieldName + ".Array.";
                ...
                mods.Insert(0,
                    new PropertyModification {
                        target = prefab, propertyPath = modificationsPath + "size",
            ...
            else
            {
                ...
                try
                {
                    ...
                    if (data.SerializedFormat == DataFormat.Nodes)
                        // Special case for node format
                    {
                        using (SerializationNodeDataReader reader = new(context))
                        using (Cache<UnityReferenceResolver> resolver = Cache<UnityReferenceResolver>.Claim())
                        {
                       ...
                       entryType != EntryType.EndOfArray && entryType != EntryType.EndOfStream)
                {
                    ...
                    if (entryType == EntryType.Invalid)
                    {
                        ...
                        try
                        {
                            ...
                            EditorApplication_delayCall_Alias += () =>
                            {
                                string log = "DELAYED SERIALIZATION LOG: Name = " +
            ...
            List<Object> referencedUnityObjects)
        {
            ...
            List<PrefabModification> result = new();
            ...
            using
                (Cache<JsonDataReader> readerCache =
            ...
            using (Cache<UnityReferenceResolver> resolver = Cache<UnityReferenceResolver>.Claim())
            {
                ...
                for (int i = 0; i < modifications.Count; i++)
                {
                    ...
                    PrefabModification modification = new();
        ...
        private static WeakValueGetter GetCachedUnityMemberGetter(MemberInfo member)
        {
            lock (UnityMemberGetters)
            {
                ...
                if (!UnityMemberGetters.TryGetValue(member, out result))
                {
        ...
        private static WeakValueSetter GetCachedUnityMemberSetter(MemberInfo member)
        {
            lock (UnityMemberSetters)
            {
                ...
                if (!UnityMemberSetters.TryGetValue(member, out result))
                {
                    if (member is FieldInfo)
                    {
        ...
        private static ICache GetCachedUnityWriter(DataFormat format, Stream stream, SerializationContext context)
        {
            ...
            switch (format)
            {
                ...
                {
                    Cache<BinaryDataWriter> c = Cache<BinaryDataWriter>.Claim();
        ...
        private static ICache GetCachedUnityReader(DataFormat format, Stream stream, DeserializationContext context)
        {
            ...
            switch (format)
            {
                case DataFormat.Binary:
                {
                    Cache<BinaryDataReader> c = Cache<BinaryDataReader>.Claim();
                    c.Value.Stream = stream;
                    cache = c;
                }
                    break;
        ...
        private static void SubscribeToDomainReloadEvents()
        {
            ...
            Delegate onBeforeDelegate = Delegate.CreateDelegate(AssemblyReloadEvents_AssemblyReloadCallback_Type,
                UnitySerializationUtility_OnBeforeAssemblyReload_Method);
            Delegate onAfterDelegate = Delegate.CreateDelegate(AssemblyReloadEvents_AssemblyReloadCallback_Type,
        ...
        private static event Action EditorApplication_delayCall_Alias
        {
            add
            {
                ...
                {
                    EditorApplication.CallbackFunction val =
                        (EditorApplication.CallbackFunction)(EditorApplication_delayCall_Member as FieldInfo)
                        .GetValue(null);
        ...
        private static T ConvertDelegate<T>(this Delegate src)
        {
            ...
            return (T)(object)src.GetInvocationList().Aggregate<Delegate, Delegate>(null,
                (current, d) => Delegate.Combine(current, (Delegate)(object)d.ConvertDelegate<T>()));
        }
