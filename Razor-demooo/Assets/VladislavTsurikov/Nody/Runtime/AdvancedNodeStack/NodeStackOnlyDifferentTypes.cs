...
namespace VladislavTsurikov.Nody.Runtime.AdvancedNodeStack
{
    public class NodeStackOnlyDifferentTypes<T> : AdvancedNodeStack<T>
        where T : Node
    {
        public sealed override void OnRemoveInvalidElements()
        {
            RemoveElementsWithSameType();
            OnRemoveInvalidElementsAfterUniqueTypeCheck();
        }

        protected virtual void OnRemoveInvalidElementsAfterUniqueTypeCheck()
        {
        }
