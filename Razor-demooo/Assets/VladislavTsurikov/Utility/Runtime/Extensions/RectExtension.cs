...
namespace VladislavTsurikov.Utility.Runtime.Extensions
{
    public static class RectExtension
    {
        ...
        {
            return new Rect(bounds.center.x - bounds.extents.x, bounds.center.z - bounds.extents.z, bounds.size.x,
                bounds.size.z);
        }
        ...
        public static Bounds CreateBoundsFromRect(Rect rect)
        {
            ...
            Vector3 center = new(rect.center.x, 0, rect.center.y);
        ...
        {
            Vector3 size = new(rect.size.x, 0, rect.size.y);
            Vector3 center = new(rect.center.x, centerY, rect.center.y);
            return new Bounds(center, size);
        }

        public static Bounds CreateBoundsFromRect(Rect rect, float centerY, float sizeY)
        {
            Vector3 size = new(rect.size.x, sizeY, rect.size.y);
            Vector3 center = new(rect.center.x, centerY, rect.center.y);
            return new Bounds(center, size);
        }
    }
