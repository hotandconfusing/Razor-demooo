...
namespace VladislavTsurikov.Utility.Runtime.Extensions
{
    public static class StringExtension
    {
        ...
        {
            StringBuilder sb = new(target.Length);
            foreach (char c in target.Where(char.IsLetterOrDigit))
            {
                sb.Append(c);
            }
        ...
        {
            return !string.IsNullOrEmpty(target) && (ignoreCase
        ...
        {
            return target.Length == 0 ? target : target.Substring(0, 1).ToUpper() + target.Substring(1).ToLower();
        }
        ...
        {
            StringBuilder sb = new(target.Length);
            foreach (char c in target.Where(c => !chars.Contains(c)))
            {
                sb.Append(c);
            }
        ...
        {
            StringBuilder sb = new(target.Length);
            foreach (char c in target.Where(c => c != symbol))
            {
                sb.Append(c);
            }
        ...
        {
            return !string.IsNullOrEmpty(target) && target.Trim()
                .Replace(" ", "")
                .All(char.IsLetter);
        }
        ...
        {
            CspParameters cspParameter = new() { KeyContainerName = key };
            RSACryptoServiceProvider rsaServiceProvider = new(cspParameter) { PersistKeyInCsp = true };
        ...
        {
            CspParameters cspParameters = new() { KeyContainerName = key };
            RSACryptoServiceProvider rsaServiceProvider = new(cspParameters) { PersistKeyInCsp = true };
