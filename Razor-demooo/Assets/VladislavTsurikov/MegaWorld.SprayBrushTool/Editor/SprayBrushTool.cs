...
using BrushSettings = VladislavTsurikov.MegaWorld.Runtime.Common.Settings.BrushSettings;
#if RENDERER_STACK
...
namespace VladislavTsurikov.MegaWorld.Editor.SprayBrushTool
{
    ...
    [MegaWorldDocUrl("tools/spray-brush")]
    public class SprayBrushTool : ToolWindow
    {
        ...
        {
            Vector3 positionForSpawn = GetRandomPointInArea(boxArea);

            RayHit rayHit = RaycastUtility.Raycast(RayUtility.GetRayFromCameraPosition(positionForSpawn),
                GlobalCommonComponentSingleton<LayerSettings>.Instance.GetCurrentPaintLayers(group.PrototypeType));
            if (rayHit != null && boxArea.Contains(rayHit.Point))
            {
                ...
                if (fitness != 0)
                {
                    SpawnTerrainObject(proto, rayHit, fitness, boxArea);
                }
            }
        }
        ...
        private void SpawnGroupGameObject(Group group, BoxArea boxArea)
        {
            ...
            if (rayHit != null && boxArea.Contains(rayHit.Point))
            {
                ...
                if (fitness != 0)
                {
                    SpawnGameObject(group, proto, rayHit, fitness, boxArea);
                }
            }
        }

        private static Vector3 GetRandomPointInArea(BoxArea boxArea)
        {
            if (boxArea.Shape == Shape.Circle)
            {
                Vector2 circleOffset = Random.insideUnitCircle * boxArea.Radius;
                return boxArea.Center + boxArea.Tangent * circleOffset.x + boxArea.Bitangent * circleOffset.y;
            }

            Vector2 squareOffset = new(Random.Range(-boxArea.Radius, boxArea.Radius),
        ...
        private static void SpawnTerrainObject(PrototypeTerrainObject proto, RayHit rayHit, float fitness, BoxArea area)
        {
#if RENDERER_STACK
            OverlapCheckSettings overlapCheckSettings =
                (OverlapCheckSettings)proto.GetElement(typeof(OverlapCheckSettings));

            Instance instance = new(rayHit.Point, proto.Prefab.transform.lossyScale, proto.Prefab.transform.rotation);
            ...
            if (!area.Contains(instance.Position))
            {
                return;
            }
