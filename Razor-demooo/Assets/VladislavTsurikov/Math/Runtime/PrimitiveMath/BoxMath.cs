...
namespace VladislavTsurikov.Math.Runtime
{
    ...
    public static class BoxMath
    {
        ...
        {
            int bestFaceIndex = 0;
            float bestScore = Vector3.Dot(direction,
                CalcBoxFaceNormal(boxCenter, boxSize, boxRotation, _allBoxFaces[0]));

            for (int faceIndex = 1; faceIndex < _allBoxFaces.Count; ++faceIndex)
            {
                float score = Vector3.Dot(direction,
            ...
            Vector3 size1, Quaternion rotation1)
        {
            ...
            Vector3 AEx = new(extents.x, extents.y, extents.z);
            extents = size1 * 0.5f;
            Vector3 BEx = new(extents.x, extents.y, extents.z);
            ...
            Vector3 t = new(Vector3.Dot(trVector, A[0]), Vector3.Dot(trVector, A[1]), Vector3.Dot(trVector, A[2]));

            // Test extents projection on box A local axes (A0, A1, A2)
            for (int axisIndex = 0; axisIndex < 3; ++axisIndex)
            {
                float bExtents = BEx[0] * absR[axisIndex, 0] + BEx[1] * absR[axisIndex, 1] +
                                 BEx[2] * absR[axisIndex, 2];
            ...
            for (int axisIndex = 0; axisIndex < 3; ++axisIndex)
            {
                float aExtents = AEx[0] * absR[0, axisIndex] + AEx[1] * absR[1, axisIndex] +
                                 AEx[2] * absR[2, axisIndex];
