...
namespace VladislavTsurikov.CPUNoise.Runtime
{
    public class WorleyNoiseCPU : NoiseCPU
    {
        ...
        public override float Sample1D(float x)
        {
            ...
            Vector3 pX = new() { [0] = Perm[pi0 - 1], [1] = Perm[pi0], [2] = Perm[pi0 + 1] };

            float f0 = float.PositiveInfinity;
            float f1 = float.PositiveInfinity;
            float f2 = float.PositiveInfinity;

            int px = Perm[(int)pX[0]];
            int py = Perm[(int)pX[1]];
            int pz = Perm[(int)pX[2]];

            float oxx = Frac(px * K) - Ko;
            float oxy = Frac(py * K) - Ko;
            float oxz = Frac(pz * K) - Ko;

            float d0 = Distance1(pf0, offsetF[0] + Jitter * oxx);
            float d1 = Distance1(pf0, offsetF[1] + Jitter * oxy);
            float d2 = Distance1(pf0, offsetF[2] + Jitter * oxz);
        ...
        public override float Sample3D(float x, float y, float z)
        {
            ...
            Vector3 pY = new() { [0] = Perm[pi1 - 1], [1] = Perm[pi1], [2] = Perm[pi1 + 1] };
