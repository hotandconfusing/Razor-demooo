...
namespace VladislavTsurikov.CustomInspector.Editor.UIToolkit
{
    ...
    {
        private readonly UIToolkitInspectorFieldsDrawer _fieldsDrawer = new(
            new List<Type>()
        );
            ...
            Action<object> onValueChanged)
        {
            ...
            Foldout foldout = new() { text = label, value = _foldout };
        ...
        private void BuildAddForm(VisualElement container, object setInstance, Action<object> onValueChanged)
        {
            ...
            VisualElement elementField =
                CreateElementField(string.Empty, _newElement, newValue => _newElement = newValue);
            fieldContainer.Add(elementField);

            Button confirmButton = new(() =>
            {
                bool exists = _containsMethod != null &&
                              (bool)_containsMethod.Invoke(setInstance, new[] { _newElement });
                if (!exists)
                {
                    try
                    {
        ...
        private void RebuildElementsList(object setInstance, Action<object> onValueChanged)
        {
            ...
            for (int i = 0; i < _elements.Count; i++)
            {
                ...
                VisualElement fieldElement = CreateElementField(string.Empty, element, newValue =>
                {
                    ...
                    bool exists = _containsMethod != null &&
                                  (bool)_containsMethod.Invoke(setInstance, new[] { newValue });
                    if (!exists)
                    {
        ...
        private void RefreshElements(object setInstance)
        {
            ...
            _elements.Clear();
            foreach (object element in (IEnumerable)setInstance)
            {
