...
namespace VladislavTsurikov.CustomInspector.Editor.Collections
{
    public sealed class HashSetFieldDrawerMatcher : FieldDrawerMatcher<IMGUIFieldDrawer>
    {
        ...
        {
            return field.FieldType.IsGenericType && field.FieldType.GetGenericTypeDefinition() == typeof(HashSet<>);
        }
    }
    ...
    public sealed class HashSetFieldDrawer : IMGUIFieldDrawer
    {
        ...
        public override object Draw(Rect rect, GUIContent label, FieldInfo field, object target, object value)
        {
            ...
            Type setType = typeof(HashSet<>).MakeGenericType(_elementType);
            PropertyInfo countProperty = setType.GetProperty("Count");
            ...
            Rect foldoutRect = new(headerRect.x, headerRect.y, headerRect.width - rightReservedWidth,
                headerRect.height);
            ...
            MethodInfo removeMethod = setType.GetMethod("Remove", new[] { _elementType });
            MethodInfo addMethod = setType.GetMethod("Add", new[] { _elementType });
            MethodInfo containsMethod = setType.GetMethod("Contains", new[] { _elementType });
            ...
            for (int i = 0; i < _elements.Count; i++)
            {
                ...
                Rect elementRect = new(contentRect.x, currentY, contentRect.width - deleteButtonWidth - 4,
                    elementHeight);
                ...
                Rect fieldRect = new(elementRect.x + 4, elementRect.y + 2, elementRect.width - 8,
                    elementRect.height - 4);
                object newElement = DrawElementField(fieldRect, _elements[i]);

                if (!Equals(newElement, _elements[i]))
                {
                    bool containsNew = containsMethod != null &&
                                       (bool)containsMethod.Invoke(value, new[] { newElement });
                    if (!containsNew)
                    {
        ...
        private void DrawAddForm(Rect rect, Type setType, object setInstance)
        {
            ...
            Rect okButtonRect = new(rect.xMax - buttonWidth - spacing - buttonWidth - spacing, currentY, buttonWidth,
            ...
            Rect labelRect = new(rect.x + 4, currentY, 70, EditorGUIUtility.singleLineHeight);
            EditorGUI.LabelField(labelRect, "Element:");

            float elementHeight = GetElementFieldHeight(_newElement);
            Rect fieldRect = new(labelRect.xMax + 4, currentY, okButtonRect.x - labelRect.xMax - 8, elementHeight);
            _newElement = DrawElementField(fieldRect, _newElement);
        }

        private float GetElementFieldHeight(object value)
        {
            if (typeof(Object).IsAssignableFrom(_elementType) || IsSimpleType(_elementType))
            {
        ...
        {
            if (typeof(Object).IsAssignableFrom(_elementType))
            {
                return EditorGUI.ObjectField(rect, value as Object, _elementType, true);
            }
