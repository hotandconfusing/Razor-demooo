...
namespace VladislavTsurikov.CustomInspector.Editor.Collections
{
    public sealed class DictionaryFieldDrawerMatcher : FieldDrawerMatcher<IMGUIFieldDrawer>
    {
        ...
        {
            return field.FieldType.IsGenericType && field.FieldType.GetGenericTypeDefinition() == typeof(Dictionary<,>);
        }
    }
    ...
    public sealed class DictionaryFieldDrawer : IMGUIFieldDrawer
    {
        ...
        public override object Draw(Rect rect, GUIContent label, FieldInfo field, object target, object value)
        {
            ...
            Type dictionaryType = typeof(Dictionary<,>).MakeGenericType(_keyType, _valueType);
            PropertyInfo keysProperty = dictionaryType.GetProperty("Keys");
            PropertyInfo valuesProperty = dictionaryType.GetProperty("Values");
            PropertyInfo countProperty = dictionaryType.GetProperty("Count");
            ...
            object values = valuesProperty.GetValue(value);
            int count = (int)countProperty.GetValue(value);

            IEnumerator keysEnumerator = ((IEnumerable)keys).GetEnumerator();
            IEnumerator valuesEnumerator = ((IEnumerable)values).GetEnumerator();
            ...
            Rect addButtonRect = new(headerRect.xMax - addButtonWidth, headerRect.y, addButtonWidth, headerRect.height);
            ...
            Rect countRect = new(headerRect.xMax - addButtonWidth - countSize.x - 10, headerRect.y, countSize.x,
                headerRect.height);
            EditorGUI.LabelField(countRect, countText);

            Rect foldoutRect = new(headerRect.x, headerRect.y, countRect.x - headerRect.x - 5, headerRect.height);
            ...
            if (_isAddingElement)
            {
                ...
                Rect addFormRect = new(contentRect.x, contentRect.y, contentRect.width, addFormHeight);
            ...
            Rect scrollViewRect = new(contentRect.x, contentRect.y, contentRect.width, scrollViewHeight);
            Rect viewRect = new(0, 0, contentRect.width - 20, totalContentHeight);

            _scrollPosition = GUI.BeginScrollView(scrollViewRect, _scrollPosition, viewRect);

            MethodInfo removeMethod = dictionaryType.GetMethod("Remove", new[] { _keyType });
            MethodInfo addMethod2 = dictionaryType.GetMethod("Add");
            ...
            for (int i = 0; i < _keys.Count; i++)
            {
                ...
                Rect elementRect = new(0, currentY, viewRect.width - deleteButtonWidth - 4, elementHeight);
                ...
                Rect removeButtonRect = new(elementRect.xMax + 4, elementRect.y + 2, deleteButtonWidth, 18);
                ...
                Rect keyFieldRect = new(elementRect.x + 4, elementRect.y + 2, elementRect.width - 8, keyHeight);
                object newKey = DrawField(keyFieldRect, _keyType, _keys[i]);

                Rect valueFieldRect = new(elementRect.x + 8, keyFieldRect.yMax + 2, elementRect.width - 12,
                    valueHeight);
        ...
        private object DrawField(Rect rect, Type fieldType, object value)
        {
            if (typeof(Object).IsAssignableFrom(fieldType))
            {
                return EditorGUI.ObjectField(rect, value as Object, fieldType, true);
            }
        ...
        private float GetAddFormHeight()
        {
            ...
            return 8 + EditorGUIUtility.singleLineHeight + keyHeight + 4 + EditorGUIUtility.singleLineHeight +
                   valueHeight + 4;
        }
        ...
        private void DrawAddForm(Rect rect, Type dictionaryType, object dictionary)
        {
            ...
            Rect confirmButtonRect = new(rect.xMax - buttonWidth - spacing - buttonWidth - spacing, currentY,
            ...
            Rect cancelButtonRect = new(rect.xMax - buttonWidth - spacing, currentY, buttonWidth,
            ...
            Rect keyLabelRect = new(rect.x + 4, currentY, 40, EditorGUIUtility.singleLineHeight);
            EditorGUI.LabelField(keyLabelRect, "Key:");

            float keyHeight = GetFieldHeight(_keyType, _newKey);
            Rect keyFieldRect = new(keyLabelRect.xMax + 4, currentY, confirmButtonRect.x - keyLabelRect.xMax - 8,
                keyHeight);
            _newKey = DrawField(keyFieldRect, _keyType, _newKey);

            currentY += keyHeight + 4;

            Rect valueLabelRect = new(rect.x + 4, currentY, 40, EditorGUIUtility.singleLineHeight);
            EditorGUI.LabelField(valueLabelRect, "Value:");

            float valueHeight = GetFieldHeight(_valueType, _newValue);
            Rect valueFieldRect = new(valueLabelRect.xMax + 4, currentY, rect.width - valueLabelRect.xMax - 8,
                valueHeight);
