    ...
    public static class FieldStyleProcessorResolver
    {
        ...
        private static void RegisterProcessors()
        {
            ...
            foreach (Assembly assembly in assemblies)
            {
                ...
                {
                    FieldStyleProcessorMatcher instance =
                        (FieldStyleProcessorMatcher)Activator.CreateInstance(matcherType);
        ...
        {
            List<FieldStyleProcessor> processors = new();
            ...
            foreach (Attribute attribute in field.GetCustomAttributes())
            {
                foreach (FieldStyleProcessorMatcher matcher in _matchers)
                {
                    ...
                    FieldStyleProcessor processor = (FieldStyleProcessor)Activator.CreateInstance(processorType);
