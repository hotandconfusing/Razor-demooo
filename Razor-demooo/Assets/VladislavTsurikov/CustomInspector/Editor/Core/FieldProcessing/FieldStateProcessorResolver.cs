    ...
    public static class FieldStateProcessorResolver
    {
        ...
        private static void RegisterProcessors()
        {
            ...
            foreach (Assembly assembly in assemblies)
            {
                ...
                {
                    FieldStateProcessorMatcher instance =
                        (FieldStateProcessorMatcher)Activator.CreateInstance(matcherType);
        ...
        {
            List<FieldStateProcessor> processors = new();
            ...
            foreach (Attribute attribute in field.GetCustomAttributes())
            {
                foreach (FieldStateProcessorMatcher matcher in _matchers)
                {
                    ...
                    FieldStateProcessor processor = (FieldStateProcessor)Activator.CreateInstance(processorType);
