...
namespace VladislavTsurikov.CustomInspector.Editor.Core
{
    public static class FieldVisibilityProcessorResolver
    {
        ...
        private static void RegisterProcessors()
        {
            ...
            foreach (Assembly assembly in assemblies)
            {
                Type[] matcherTypes = assembly.GetTypes()
                    .Where(t => typeof(FieldVisibilityProcessorMatcher).IsAssignableFrom(t) && !t.IsAbstract &&
                                t.IsClass)
                    .ToArray();

                foreach (Type matcherType in matcherTypes)
                {
                    FieldVisibilityProcessorMatcher instance =
                        (FieldVisibilityProcessorMatcher)Activator.CreateInstance(matcherType);
        ...
        {
            List<FieldVisibilityProcessor> processors = new();
            ...
            foreach (Attribute attribute in field.GetCustomAttributes())
            {
                foreach (FieldVisibilityProcessorMatcher matcher in _matchers)
                {
                    ...
                    FieldVisibilityProcessor processor =
                        (FieldVisibilityProcessor)Activator.CreateInstance(processorType);
