    ...
    public static class FieldValueProcessorResolver
    {
        ...
        private static void RegisterProcessors()
        {
            ...
            foreach (Assembly assembly in assemblies)
            {
                ...
                {
                    FieldValueProcessorMatcher instance =
                        (FieldValueProcessorMatcher)Activator.CreateInstance(matcherType);
        ...
        {
            List<FieldValueProcessor> processors = new();
            ...
            foreach (Attribute attribute in field.GetCustomAttributes())
            {
                foreach (FieldValueProcessorMatcher matcher in _matchers)
                {
                    ...
                    FieldValueProcessor processor = (FieldValueProcessor)Activator.CreateInstance(processorType);
