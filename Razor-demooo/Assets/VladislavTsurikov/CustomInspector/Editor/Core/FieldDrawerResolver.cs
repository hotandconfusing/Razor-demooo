    ...
    public static class FieldDrawerResolver<TDrawer> where TDrawer : FieldDrawer
    {
        ...
        private static void RegisterDrawers()
        {
            ...
            foreach (Assembly assembly in assemblies)
            {
                ...
                {
                    FieldDrawerMatcher<TDrawer> instance =
                        (FieldDrawerMatcher<TDrawer>)Activator.CreateInstance(matcherType);
        ...
        public static TDrawer CreateDrawer(FieldInfo field)
        {
            ...
            HashSet<Type> fieldAttributeTypes = GetFieldDrawerAttributeTypes(field);
            ...
            foreach (FieldDrawerMatcher<TDrawer> matcher in _matchers)
            {
                ...
                IReadOnlyList<Type> matcherAttributeTypes = matcher.AttributeTypes;
        ...
        private static void RegisterDrawerAttributes(FieldDrawerMatcher<TDrawer> matcher)
        {
            ...
            IReadOnlyList<Type> attributeTypes = matcher.AttributeTypes;
        ...
        {
            HashSet<Type> fieldAttributeTypes = new();
        ...
        {
            if (_typeOnlyFields.TryGetValue(fieldType, out FieldInfo field))
            {
                return field;
            }

            Type holderTypeDefinition = typeof(TypeOnlyField<>);
            int holderTypeArgsCount = holderTypeDefinition.GetGenericArguments().Length;
            Type holderType;
